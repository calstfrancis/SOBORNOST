...
export function saveGameSlot(slotId) {
  try {
    const state = {
      ...
      cover: G.cover, coverIntegrity: G.coverIntegrity,
      notes: G.notes, flags: [...G.flags],
      scene: G.scene, mode: G.mode, lastReaction: G.lastReaction,
      tutorialDone: G.tutorialDone, crossingLog: G.crossingLog,
      inventory: G.inventory, time: G.time,
      reputation: G.reputation, quests: G.quests, awareness: G.awareness,
      beliefs: [...G.beliefs], knowledge: [...G.knowledge],
      consequenceQueue: G.consequenceQueue, npcStance: G.npcStance,
      eventQueue: G.eventQueue, pastLifeFlags: [...G.pastLifeFlags],
    ...
    // Upgrade 4: journal saved separately (it survives across play resets)
    saveJournal();
...
export function loadGameSlot(slotId) {
  try {
    ...
    G.phase = 'game';
    // Upgrade 4: load journal from its own key
    loadJournal();
    refreshAtmosMods();
    scheduleRender();
